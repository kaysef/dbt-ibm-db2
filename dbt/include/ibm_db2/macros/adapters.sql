...
{% macro ibm_db2__create_schema(relation) -%}
...
{% macro ibm_db2__list_relations_without_caching(schema_relation) %}
...
{% macro ibm_db2__list_schemas(database) %}
...
{% macro ibm_db2__drop_relation(relation) -%}
...
{% macro ibm_db2__rename_relation(from_relation, to_relation) -%}
...
{% macro ibm_db2__current_timestamp() -%}
  CURRENT_TIMESTAMP
{%- endmacro %}

{% macro ibm_db2__get_columns_in_relation(relation) -%}
...
{% macro ibm_db2__make_temp_relation(base_relation, suffix) %}
...
{% macro ibm_db2__alter_relation_comment(relation, comment) %}
...
{% macro ibm_db2__alter_column_comment(relation, column_dict) %}
...
{% macro ibm_db2__persist_docs(relation, model, for_relation, for_columns) -%}
...
{% macro ibm_db2__alter_column_type(relation, column_name, new_column_type) -%}
  {#
