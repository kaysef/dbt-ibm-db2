...
{% macro ibm_db2__create_csv_table(model, agate_table) %}
...
{% macro ibm_db2__reset_csv_table(model, full_refresh, old_relation, agate_table) %}
    {% set sql = "" %}
...
{% macro ibm_db2__basic_load_csv_rows(model, max_batch_size, agate_table) %}
...
{% macro ibm_db2__load_csv_rows(model, agate_table) %}
  {% set max_batch_size = var("max_batch_size", 400) %}
  {{ return(ibm_db2__basic_load_csv_rows(model, max_batch_size, agate_table) )}}
{% endmacro %}


{% materialization seed, adapter='ibm_db2' %}
